...
Para cada ítem indica resultado: "C" (Cumple), "NC" (No Cumple) o "C_OBS" y una nota breve.
...
export async function POST(req: NextRequest, { params }: { params: { etiquetaId: string } }) {
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          { type: 'text', text: `Analiza las etiquetas del producto "${registro.producto}", lote ${registro.lote}.` },
        ],
      }],
    })
    ...
    if (error instanceof SyntaxError)
      return NextResponse.json({ error: 'La IA devolvió una respuesta no parseable.' }, { status: 502 })
