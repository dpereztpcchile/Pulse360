  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] font-[Rajdhani] text-[#e8e8e8] flex flex-col max-w-lg mx-auto">
      ...
      {paso === 1 && registro && (
        <>
          <div className="flex-1 p-4 overflow-y-auto">
            ...
            {itemsNC > 0 && <div className="bg-[#0a0800] border border-[#3a2a00] rounded-lg px-3 py-2 flex gap-2 mb-3"><span className="text-[#d4a017]">⚠</span><span className="text-[12px] text-[#d4a017]">{itemsNC} ítem(s) con NC.</span></div>}
            ...
            <div className="grid grid-cols-2 gap-2">
              {[{ rol: 'CALIDAD', nombre: firmaCalidad?.nombreUsuario ?? 'Analista', firmado: !!firmaCalidad, esMia: rolFirma === 'CALIDAD' }, { rol: 'SUPERVISOR', nombre: firmaSuperviser?.nombreUsuario ?? 'Supervisor', firmado: !!firmaSuperviser, esMia: rolFirma === 'SUPERVISOR' }].map(f => (
                <div key={f.rol} className={`border rounded-lg p-3 ${f.firmado ? 'border-[#16a34a] bg-[#040e06]' : f.esMia ? 'border-[#4a9eff] bg-[#040810]' : 'border-[#2a2a2a]'}`}>
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${f.firmado ? 'bg-[#0a2a0a] text-[#16a34a]' : f.esMia ? 'bg-[#060e1a] text-[#4a9eff]' : 'bg-[#2a1f00] text-[#d4a017]'}`}>
                    {f.firmado ? 'AUTORIZADO ✓' : f.esMia ? 'TU FIRMA' : 'PENDIENTE'}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#1a1a1a] border-t border-[#1e1e1e] p-4">
            <button onClick={() => setPaso(2)} className="w-full py-4 rounded-lg bg-[#CC0000] text-white font-bold text-base">Revisar →</button>
          </div>
        </>
      )}
      ...
      {paso === 2 && registro && (
        <>
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="text-[10px] font-bold tracking-[2px] text-[#CC0000] uppercase mb-3 pb-1 border-b border-[#1e1e1e]">Fotografías</div>
              ...
              {(['ETIQUETA_PT', 'MATERIA_PRIMA'] as const).map(tipo => {
                ...
                return (
                  <div key={tipo} className="border border-[#2a2a2a] rounded-xl overflow-hidden">
                    <div className="bg-[#1e1e1e] px-3 py-2"><span className="text-[10px] font-bold text-[#666] uppercase">{label}</span></div>
          ...
          <div className="bg-[#1a1a1a] border-t border-[#1e1e1e] p-4 flex gap-3">
            ...
            <button onClick={() => setPaso(3)} className="flex-[2] py-4 rounded-lg bg-[#CC0000] text-white font-bold text-base">Checklist →</button>
          </div>
        </>
      )}
      ...
      {paso === 3 && registro && (
        <>
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="text-[10px] font-bold tracking-[2px] text-[#CC0000] uppercase mb-3 pb-1 border-b border-[#1e1e1e]">Checklist</div>
            {registro.checklist.map(item => {
              const cls = item.resultado === 'NC' ? 'border-l-[3px] border-l-[#CC0000] bg-[#130505]' : 'border-l-[3px] border-l-[#16a34a]'
              const badgeCls = item.resultado === 'NC' ? 'bg-[#2a0a0a] text-[#CC0000]' : 'bg-[#0a2a0a] text-[#16a34a]'
              return (
                <div key={item.itemKey} className={`bg-[#1a1a1a] border border-[#222] rounded-lg p-3 mb-1.5 flex items-center justify-between ${cls}`}>
                  <div className="flex-1 mr-2"><div className="text-sm font-bold text-[#d0d0d0]">{item.itemNombre}</div></div>
      ...
      {paso === 4 && registro && (
        <>
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="text-[10px] font-bold tracking-[2px] text-[#CC0000] uppercase mb-3 pb-1 border-b border-[#1e1e1e]">Decisión</div>
            ...
            <div className="bg-[#1a1a1a] border border-[#222] rounded-lg p-3 mb-4">
              <textarea value={observacion} onChange={e => setObservacion(e.target.value)} placeholder="Observación opcional..." className="w-full bg-transparent text-[#e0e0e0] text-sm outline-none resize-none min-h-[48px] placeholder:text-[#333]" />
            </div>
            {rechazando && (
              <div className="bg-[#1a0505] border border-[#3a1010] rounded-lg p-3 mb-4">
                <div className="text-[11px] font-bold text-[#CC0000] mb-2">Motivo de rechazo</div>
                <textarea value={motivoRechazo} onChange={e => setMotivoRechazo(e.target.value)} placeholder="Describe el motivo..." className="w-full bg-transparent text-[#e0e0e0] text-sm outline-none resize-none min-h-[64px]" autoFocus />
              </div>
            )}
          </div>
          <div className="bg-[#1a1a1a] border-t border-[#1e1e1e] p-4">
            {!rechazando ? (
              <div className="flex gap-3">
                ...
                <button onClick={handleAutorizar} disabled={loading || yaFirme} className="flex-[2] py-4 rounded-lg bg-[#16a34a] text-white font-bold text-base disabled:opacity-40">{loading ? 'Autorizando...' : '✓ Autorizar'}</button>
              </div>
            ) : (
              <div className="flex gap-3">
...
export default function RevisionTablet() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0f0f0f] flex items-center justify-center font-[Rajdhani] text-[#555]">Cargando...</div>}>
      <RevisionContent />
    </Suspense>
  )
}
